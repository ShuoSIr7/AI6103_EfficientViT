...
from model.model import MyEfficientViT
...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data-path', default='/root/autodl-tmp/cifar-100', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='CIFAR', choices=['CIFAR', 'IMNET1000', 'IMNET100'],
...
def validate_step(model, valid_loader, device):
    ...
    with torch.no_grad():
        for inputs, targets in valid_loader:
            ...
            print("throughput: ", val_throughput)
...
def main(args):
    ...
    model = MyEfficientViT(num_classes=args.nb_classes, img_size=args.input_size).to(device)
    ...
    with open('training.logs', 'w') as log_file:
        ...
        for epoch in range(args.epochs):
            ...
            log_file.write(f"{epoch},{val_loss},{val_acc},{val_throughput}\n")
